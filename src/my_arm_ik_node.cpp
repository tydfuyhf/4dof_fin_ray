...
public:
  MyArmIkNode() : Node("my_arm_ik_node") {
    ...
    pub_desired_ = create_publisher<sensor_msgs::msg::JointState>("/desired_joint_states", 10);
...
private:
  void buildChain() {
    ...
    // gripper_link2 -> joint_without_idler2 (fixed)
    segments_.push_back({{-0.017, -0.0352, -0.0115}, {1.5708, -1.5708, 0}, false, {0, 0, 0}, "", 0, 0});

    // joint_without_idler2 -> gripper_base (fixed)
