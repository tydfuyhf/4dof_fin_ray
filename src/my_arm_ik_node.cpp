...
public:
  MyArmIkNode():Node("my_arm_ik_node"){
    ...
    RCLCPP_INFO(get_logger(),"IK node ready. Waiting for /tip_target...");
  }
...
private:
  ...
  void onTick(){
    ...
    if(!have_target_) return;   // target 오기 전까지 아무것도 안함
...
private:
  double rate_hz_,lambda_,step_gain_,tol_m_;
  int ik_iters_per_tick_;
  ...
  Eigen::Vector3d desired_target_;
